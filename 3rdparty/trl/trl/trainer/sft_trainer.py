...
class SFTTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        shifted_labels = inputs["labels"][:, 1:].contiguous().clone()
        shifted_labels[:,-1] = inputs["input_ids"][:, -1].clone()
        think_token_id = self.processing_class.convert_tokens_to_ids("<think>")
        think_end_token_id = self.processing_class.convert_tokens_to_ids("</think>")
        think_start_idx = (shifted_labels == think_token_id).nonzero()
        think_end_idx = (shifted_labels == think_end_token_id).nonzero()
        if think_start_idx.shape[0] == 0:
            print(">>>>>>>>>> No <think> token found in the input.")
            ib, think_start_idx = 0, 0
        else:
            ib, think_start_idx = think_start_idx[0].tolist()
        if think_end_idx.shape[0] == 0:
            ib, think_end_idx = 0, shifted_labels.shape[1]
        else:
            ib, think_end_idx = think_end_idx[0].tolist()
        for x in inputs.keys():
            if isinstance(inputs[x], torch.Tensor):
                inputs[x] = inputs[x][:, :self.max_length]
        inputs["labels"][:, -1] = -100
        inputs["total_length"] = total_length
        inputs["think_start_idx"] = think_start_idx
        inputs["think_end_idx"] = think_end_idx
        del shifted_labels
        ...
        if "labels" in inputs and not self.args.use_liger:
            ...
            if hasattr(outputs, "pred_abs_error"):
                pred_abs_error = outputs.pred_abs_error
                pred_prob = outputs.pred_prob
                pred_prob_last_10 = pred_prob[-pred_prob.shape[0] // 10:]
                pred_prob_last_25 = pred_prob[-pred_prob.shape[0] // 4:]
                pred_prob_last_50 = pred_prob[-pred_prob.shape[0] // 2:]
                self._metrics["mean_pred_prob_last_10"].append(pred_prob_last_10.mean().item())
                self._metrics["mean_pred_prob_last_25"].append(pred_prob_last_25.mean().item())
                self._metrics["mean_pred_prob_last_50"].append(pred_prob_last_50.mean().item())
                self._metrics["mean_pred_prob"].append(pred_prob.mean().item())
                pred_abs_error_last_10 = pred_abs_error[-pred_abs_error.shape[0] // 10:]
                pred_abs_error_last_25 = pred_abs_error[-pred_abs_error.shape[0] // 4:]
                pred_abs_error_last_50 = pred_abs_error[-pred_abs_error.shape[0] // 2:]
                mean_abs_error = pred_abs_error[pred_abs_error < torch.quantile(pred_abs_error, 0.9)].mean().item()
                mean_abs_error_last_10 = pred_abs_error_last_10[pred_abs_error_last_10 < torch.quantile(pred_abs_error_last_10, 0.9)].mean().item()
                mean_abs_error_last_25 = pred_abs_error_last_25[pred_abs_error_last_25 < torch.quantile(pred_abs_error_last_25, 0.9)].mean().item()
                mean_abs_error_last_50 = pred_abs_error_last_50[pred_abs_error_last_50 < torch.quantile(pred_abs_error_last_50, 0.9)].mean().item()
                self._metrics["mean_abs_error"].append(mean_abs_error)
                self._metrics["mean_abs_error_last_10"].append(mean_abs_error_last_10)
                self._metrics["mean_abs_error_last_25"].append(mean_abs_error_last_25)
                self._metrics["mean_abs_error_last_50"].append(mean_abs_error_last_50)
