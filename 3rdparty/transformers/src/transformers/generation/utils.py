...
class GenerationMixin:
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        token_budget = torch.tensor(token_budget, device=input_ids.device).float()
        max_token_budget = token_budget.clone()
        apply_budget = (token_budget > 0).item()
        is_prefill = True
        prev_token_is_newline = True
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            # prepare model inputs
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            ...
            _apply_budget = prev_token_is_newline and apply_budget
            model_inputs.update({"apply_budget": _apply_budget} if _apply_budget else {})

            if is_prefill:
                outputs = self(**model_inputs, return_dict=True)
                is_prefill = False
                # if apply_budget:
                #     self._expected_L = self.decode_budget((outputs.alpha - 1) / outputs.lam, "log")
                # return expected_L[self.tokenizer.convert_tokens_to_ids("Okay")]
            else:
            ...
            if prev_token_is_newline and apply_budget:
                next_token_scores = next_token_logits.clone()
            else:
                # pre-process distribution
                next_token_scores = logits_processor(input_ids, next_token_logits)
            ...
            if prev_token_is_newline and apply_budget:
                keep_lam = outputs.lam
                keep_alpha = outputs.alpha
                budget_cond_probs = self.gamma_length_prob(torch.ones(keep_alpha.shape, device=keep_alpha.device) * token_budget, keep_alpha, keep_lam, "log", interval=-1, max_value=max_token_budget).clamp(min=0.0)
                expected_L = self.decode_budget((keep_alpha - 1) / keep_lam, "log")
                budget_cond_probs *= (keep_lam >= 10.0) & (keep_alpha > 1.0) & (expected_L >= 0.9)
                if (budget_cond_probs == 0).all():
                    budget_cond_probs[next_token_logits[0].argmax()] = 1.0
                budget_cond_probs = budget_cond_probs.clamp_(min=0.0, max=1.0)
                budget_cond_probs = budget_cond_probs / budget_cond_probs.sum()
                probs[0] *= budget_cond_probs
            ...
            if apply_budget:
                token_budget = (token_budget-1).clamp(min=1)

            if "\n" in self.tokenizer.decode(next_tokens) and apply_budget:
                prev_token_is_newline = True
            else:
                prev_token_is_newline = False

            if "</think>" in self.tokenizer.decode(next_tokens):
                apply_budget = False
